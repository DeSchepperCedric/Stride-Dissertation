...
TEST(DaycareGeneratorTest, OneLocationTest)
{
        ...
        DaycareGenerator daycareGenerator(rnMan, CreateTestLogger());
        GeoGridConfig    config{};

        IdSubscriptArray<unsigned int> contactCenterCounter(1U);

        config.input.pop_size           = 10000;
        config.popInfo.popcount_daycare = 300;
        ...
        daycareGenerator.Apply(geoGrid, config, contactCenterCounter);

        const auto& centersOfLoc1 = loc1->RefCenters(Id::Daycare);
...
TEST(DaycareGeneratorTest, FiveLocationsTest)
{
        RnMan              rnMan{RnInfo()}; // Default random number manager.
        DaycareGenerator daycareGenerator(rnMan, CreateTestLogger());
        GeoGridConfig      config{};
        ...
        config.popInfo.popcount_daycare = 125140;
        ...
        const auto& centersOfLoc1 = loc1->RefCenters(Id::Daycare);
        EXPECT_EQ(centersOfLoc1.size(), 12193);

        const auto& centersOfLoc2 = loc2->RefCenters(Id::Daycare);
        EXPECT_EQ(centersOfLoc2.size(), 11784);

        const auto& centersOfLoc3 = loc3->RefCenters(Id::Daycare);
        EXPECT_EQ(centersOfLoc3.size(), 8890);

        const auto& centersOfLoc4 = loc4->RefCenters(Id::Daycare);
        EXPECT_EQ(centersOfLoc4.size(), 3934);

        const auto& centersOfLoc5 = loc5->RefCenters(Id::Daycare);
        EXPECT_EQ(centersOfLoc5.size(), 4913);
}
} // namespace
