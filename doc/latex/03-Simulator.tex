...
%
    \item Configuration files (xml and json) in directory \texttt{<install\_dir>/config}
      	\begin{compactitem}
			...
			\item $run\_generate\_regions\_json.xml$: Adapted version of $run\_generate\_default.xml$ to use different regions.
        		\item \ldots
        \end{compactitem}
%        
    \item Data files (csv) in directory \texttt{<project\_dir>/data}
      	\begin{compactitem}
        	...
        	\item $flanders\_major\_cities.csv$: ID's of the central cities in Flanders.
