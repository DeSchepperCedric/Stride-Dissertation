...
void Generator<stride::ContactType::Id::Daycare>::Apply(GeoGrid& geoGrid, const GeoGridConfig& ggConfig)
{
        ...
        for (const auto& it : ggConfig.regionsInfo) {
                ...
                const auto majorPupilCount =
                    static_cast<unsigned int>(ceil(it.second.major_fraction_daycare * majorPopCount));
                const auto majorSchoolCount = static_cast<unsigned int>(
                    round(majorPupilCount / static_cast<double>(ggConfig.people[Id::Daycare])));
                const auto pupilCount = static_cast<unsigned int>(ceil(it.second.fraction_daycare * popCount));
                const auto schoolCount =
                    static_cast<unsigned int>(round(pupilCount / static_cast<double>(ggConfig.people[Id::Daycare])));
