                    ...
                    if (isActiveWorker) {
                        ...
                        if (!commuteLocations.empty() && isCommuter) {
                            ...
                            if (wp_types_present) {
                                ...
                                // if no workplace with room left found, calculate weights to find best fit
                                if(defaultIndex == foundPoolIndex){
                                    ...
                                    foundPoolIndex = m_rn_man.GetDiscreteGenerator(weightsCommutePools, 0U)();
                                }
                            ...
                            if (wp_types_present) {
                                ...
                                for (int i = 0; i < s; i++) {
                                    ...
                                    if(nearbyWp[i]->size() < geoGridConfig.refWP.max[index]){
                                        foundPoolIndex = i;
                                        break;
                                    }
                                }
                                // if no workplace with room left found, calculate weights to find best fit to add one
                                if(defaultIndex == foundPoolIndex){
                                    std::vector<double> weightsCommutePools;
                                    for (int i = 0; i <  s; i++) {
                                        const unsigned int index = geopop::GetWorkplaceIndex(
                                                poolTypes, nearbyWp[i]->GetId(), genWorkPlaceSize);
                                        const auto weight =
                                                GetWorkplaceWeight(geoGridConfig, nearbyWp[i], index);
                                        weightsCommutePools.push_back(weight);
                                        AssertThrow(
                                                weight >= 0.0 && weight <= 1.0 && !isnan(weight),
                                                "Invalid weight: " + to_string(weight), m_logger);
